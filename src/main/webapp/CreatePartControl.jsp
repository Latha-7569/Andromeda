...
document.addEventListener('DOMContentLoaded', async () => {
	  ...
	  // Load dropdown data
	  try {
	    const res = await fetch('http://localhost:8080/navigator/api/excel/dropdowns');
	    ...
	    // Pre-fill assignee from logged-in user
	  ...
	  // On supertype change, populate type dropdown
	  ...
	  form.addEventListener('submit', async (e) => {
	    ...
	    try {
	      const res = await fetch('http://localhost:8080/navigator/api/datafetchservice/createpart', {  
	        method: 'POST',
	        headers: { 
	      ...
	      try {
	        ...
	        alert('Submission failed: Server returned an unexpected response.');
	        return;
	      }
